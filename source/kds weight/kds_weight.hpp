...
#include "../utils/kdtree_weight.hpp"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <limits.h>
...
public:
    ...
    kds_weight()
    {
        ...
        root = kdtree(datapoints, size - 1, 0);
    ...
    void independent_range_sampling(const std::pair<unsigned int, unsigned int>& interval)
    {
        ...
        if (result.size() > 0)
        {
            ...
            for (unsigned int i = 0; i < result.size(); ++i)
            {
                ...
                {
                    arr.push_back(result[i].first->location.weight);
                }
                else
                {
                    arr.push_back(result[i].first->weight_sum);
                }
            }
            alias b(arr);
            ...
            {
                ++sampling_num;
                // std::cout << " sampling iteration: " << sampling_num << "\n";
                ...
                if (result[idx].second == 1)
                {
                    while (1)
                    {
                        if (v->leftChild == NULL && v->rightChild == NULL) break;

                        arr.clear();
                        arr.push_back(v->location.weight);
                        if (v->leftChild != NULL) arr.push_back(v->leftChild->weight_sum);
                        if (v->rightChild != NULL) arr.push_back(v->rightChild->weight_sum);
                        alias c(arr);
